...
namespace AdityaZanjad\HttpWrapper\Providers\Curl;

use AdityaZanjad\HttpWrapper\Interfaces\HttpRequest;
use Exception;
...
class Request implements HttpRequest
{
    ...
    public function build(): array
    {
        ...
        if (isset($this->request['headers']) && !empty($this->request['headers'])) {
            $this->req[CURLOPT_HTTPHEADER] = \array_map(fn ($value, $name) => "{$name}: {$value}", $this->request['headers']);
        }
        ...
        foreach ($this->request['body']['content'] as $field) {
            if (\is_string($field['value'])) {
                if (\is_file($field['value'])) {
                    $body[$field['label']] = new \CURLFile($field['value'], $field['mime'] ?? null, $field['name'] ?? $field['label']);
                }
